...
Optimized: skips providers with missing API keys instantly.
"""
...
# ─── Cache API keys at import time (avoid re-reading env every call) ─────────
...
def get_llm_response(prompt, system_prompt=None):
    """
    Get LLM response with smart fallback chain.
    Skips providers with missing API keys instantly.
    ...
    """
    # Try OpenAI first (skipped instantly if no key)
    if _openai_key:
        result = call_openai(prompt, system_prompt)
        if result:
            return result

    # Try Gemini (skipped instantly if no key)
    if _gemini_key:
